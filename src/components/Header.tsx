...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'shadow-lg bg-[#fdf7f2]' : 'bg-transparent'
        }`}
    >
      <div className="w-full px-6 lg:px-10 h-16">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="flex-shrink-0">
            <h1 className={`font-playfair text-2xl lg:text-3xl font-bold transition-colors duration-300 ${isScrolled ? 'text-gray-900' : 'text-white'}`}>
              Istyaq Facility
            </h1>
            <p className={`text-xs font-medium transition-colors duration-300 ${isScrolled ? 'text-gray-700' : 'text-white'}`}>
          ...
          <div className="hidden lg:flex items-center space-x-6 ml-auto">
            ...
            <div className="flex items-center space-x-2 mr-4">
              <Phone
                className={`h-5 w-5 transition-colors duration-300 ${isScrolled ? 'text-gray-900' : 'text-white'}`}
              />
              <span
            ...
            <Link
              to="/contact"
              className="bg-white text-black font-semibold px-5 py-2 rounded-full flex items-center space-x-2 hover:bg-gray-200 transition"
            >
